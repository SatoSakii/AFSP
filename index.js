...
function acceptFriend(userID, username, discriminator) {
...
function searchingFriendRequest() {
    console.log("\n\x1b[90m[\x1b[93m/\x1b[90m] \x1b[97mSearching for friend request(s)...\x1b[0m");
    let i = 0;
    ...
    request({
        ...
        if(JSON.parse(body)) {
            JSON.parse(body).forEach(e => {
                if(e.type && e.type == 3) {
                    i++;
                    acceptFriend(e.user.id, e.user.username, e.user.discriminator);
                };
            });
            if(i == 0) {
...
async function StartScript() {
    ...
    process.title = "AFS P | by Sato.";

    console.log("AFS P Loading...\x1b[0m");
    setTimeout(async () => {
        console.clear();
